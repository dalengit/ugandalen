...
export function ChaosLayer({ blessings }: { blessings: number }) {
  ...
  return (
    <>
      ...
      <div className="falling-flags" aria-hidden="true">{Array.from({ length: 16 }, (_, i) => <span key={i} style={{ "--i": i } as React.CSSProperties}><img src={asset("Flag-Uganda.webp")} alt="" /></span>)}</div>
      <div className="cursor-trail" aria-hidden="true">{trail.map((dot) => <img key={dot.id} src={asset("ugandalen_portrait.png")} alt="" style={{ left: dot.x, top: dot.y }} />)}</div>
      <div className="bouncer" aria-hidden="true"><img src={asset("ugandalen_portrait.png")} alt="" /></div>
